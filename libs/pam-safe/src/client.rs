...
impl<'a, C: conv::Conversation> Client<'a, C> {
    ...
    fn initialize_environment(&mut self) -> PamResult<()> {
        ...
        let user = match User::from_name(self.conversation.username()) {
            Ok(Some(user)) => user,
            Ok(None) => return Err(PamReturnCode::User_Unknown.into()),
            Err(_) => return Err(PamReturnCode::System_Err.into()),
        };
        let home = user
            .dir
            .to_str()
            .ok_or_else(|| PamError(PamReturnCode::System_Err))?;
        let shell = user
            .shell
            .to_str()
            .ok_or_else(|| PamError(PamReturnCode::System_Err))?;
    ...
    // Utility function to set an environment variable in this PAM session.
