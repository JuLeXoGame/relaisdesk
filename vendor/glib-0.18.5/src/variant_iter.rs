...
#[cfg(test)]
mod relaisdesk_security_tests {
    use crate::ToVariant;

    #[test]
    fn variant_str_iter_security_regression() {
        let strings = ["", "écran", "第三", "last"];
        ...
        assert_eq!(variant.array_iter_str().unwrap().nth(1), Some("écran"));
        assert_eq!(variant.array_iter_str().unwrap().nth_back(1), Some("第三"));
        ...
        assert_eq!(iter.next(), Some("écran"));
        assert_eq!(iter.next_back(), Some("第三"));
